program swimming_plate
  ...
  !
    ...
    real, parameter    :: finish=16
    integer            :: b(3) = [2,2,1]
  !
  ! -- Hyperparameters
    real, parameter    :: thicc=0.03*L
    real, parameter    :: A = 0.1*L, St_d = 0.3, k_x=0., k_z=256.0, h_roughness=0.0
  ...
  ! -- Setup solver
    ...
    !
      time_loop: do while (flow%time<finish/f.and..not.there)
        ...
        if((t>(finish-8)/f).and.(mod(t,0.020/f)<dt)) call flow%write(geom, write_vtr=.false.)
      end do time_loop
